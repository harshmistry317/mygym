...
class set_deit_admin : AppCompatActivity() {
    lateinit var databaseReference: DatabaseReference
    lateinit var recyal : RecyclerView
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_set_deit_admin)
        var listofuser = ArrayList<Userdata>()
        recyal = findViewById(R.id.recyclerview_setdiet)
        ...
        databaseReference.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot!!.exists()){
                    ...
                    var adapterupdate = SetdeitAdpter(listofuser,application)
